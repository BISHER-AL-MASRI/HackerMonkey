...
namespace HackerMonkey.Displays
{
    /// <summary>
    ...
    /// </summary>
    public class HackerMonkeyMutliDisplay : ModTowerDisplay<HackerMonkeyMod>
    {
        private readonly int[] t = null!; // The tiers used for a particular instance of HackerMonkeyBaseDisplay

        /// <summary>
        /// All classes that derive from ModContent MUST have a zero argument constructor to work
        /// </summary>

        public HackerMonkeyMutliDisplay(int[] tiers)
        {
            t = tiers;
        }
        ...
        public override string Name => nameof(HackerMonkeyMutliDisplay) + "-" + t.Printed();
            ...
            for (var i = 0; i <= 2; i++)
            {
                for (var j = 0; j <= 2; j++)
                {
                    for (var k = 0; k <= 2; k++)
                    {
                        ...
                        if (tiers.IsValid())
                        {
                            yield return new HackerMonkeyMutliDisplay(tiers);
                        }
                    }
                }
            }
...
# if DEBUG
            ...
            SetMeshTexture(node, nameof(HackerMonkeyBaseDisplay));
